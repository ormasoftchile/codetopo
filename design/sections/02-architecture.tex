...
in \S\ref{sec:data-model}. SQLite WAL mode\footnote{In SQLite's default
DELETE journal mode any write transaction blocks all readers. WAL
(Write-Ahead Log) mode permits concurrent readers alongside one active
writer; readers see a consistent snapshot of the last committed state.}
allows the Query Engine to serve
