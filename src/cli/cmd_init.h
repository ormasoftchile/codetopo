...
inline bool write_copilot_instructions(const std::filesystem::path& repo_root) {
    ...
    static const char* content =
        ...
        "**Enumeration (survey first):** `symbols_in_path` is compact by default — returns `node_id`, `name`, "
        "`kind`, `span` only. Use the returned `node_id`s for follow-up calls; they stay valid across the session.\n\n"
        "**Call graph work:** use `context_for(node_id, include_source=false)` to get callers/callees without "
        "the source body (~90% fewer tokens). Then fetch source only for the specific lines you need with `source_at`.\n\n"
        "**Targeted source reads:** prefer `source_at(file, start_line, end_line)` over reading whole files. "
        "A single function body is typically 20-80 lines; reading the whole file wastes tokens.\n\n"
        "**Avoid re-reading:** `node_id`s returned by any tool are reusable handles. "
        "Never re-run `symbol_search` for a symbol you already have a `node_id` for.\n\n"
        "## Symbol kinds\n\n"
