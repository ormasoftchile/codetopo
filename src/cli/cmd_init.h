...
inline bool write_copilot_instructions(const std::filesystem::path& repo_root) {
    ...
    static const char* content =
        ...
        "**Enumeration:** `symbols_in_path` is compact by default — pass `compact=true` explicitly for clarity. "
        "Returns only `node_id`, `name`, `kind`, `span`. Use the `node_id`s for all follow-up calls.\n\n"
