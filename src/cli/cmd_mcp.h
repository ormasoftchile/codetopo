...
inline int run_mcp(const std::string& db_path, const std::string& root_hint,
    ...
    server.register_tool("callers_approx", tools::callers_approx,
        "Find exact callers from call graph edges. If exact callers are empty, also returns refs-backed candidate_results for overloaded/common member calls such as .set; use include_candidates or mode='exact_plus_candidates' to always include them.",
        R"J({"type":"object","properties":{"node_id":{"type":"integer","description":"The node_id of the symbol to find callers for"},"group_by":{"type":"string","enum":["file","module","symbol"],"description":"Group exact results by file path, module (directory), or symbol name. Omit for flat list."},"compact":{"type":"boolean","description":"Compact output for exact results: use generic symbol fields, omit qualname=name, collapse span to [start,end], and use 'file' instead of 'file_path'."},"include_candidates":{"type":"boolean","description":"When true, always add refs-backed candidate_results; when false, return exact edges only. Omitted means add candidates only if exact results are empty."},"mode":{"type":"string","enum":["exact","exact_then_candidates","exact_plus_candidates"],"description":"Candidate collection mode. Default exact_then_candidates keeps exact edges in results/groups and puts approximate refs matches in candidate_results only when exact edges are empty."},"limit":{"type":"integer","description":"Max exact results and candidate results (default 50, max 500)"}},"required":["node_id"]})J");
