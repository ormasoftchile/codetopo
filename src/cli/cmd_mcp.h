...
inline int run_mcp(const std::string& db_path, const std::string& root_hint,
    ...
    server.register_tool("callers_approx", tools::callers_approx,
        "Find exact callers from call graph edges. If exact callers are empty, also returns lean refs-backed candidate_results for overloaded/common member calls such as .set; use response_mode='lean' for grep-weight summaries.",
